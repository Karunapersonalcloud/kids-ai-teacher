...
5. Confirm the temporary PIN appears and the admin panel reports whether login instructions were emailed.
6. Login at `/login`.
7. Confirm first login redirects to `/change-credentials`.
8. Confirm AI usage/progress updates are saved.

Approval emails are sent with Resend when `EMAIL_PROVIDER=resend`, `RESEND_API_KEY`, and `EMAIL_FROM` are configured. To send from `ConceptKid <no-reply@mail.conceptkid.in>`, configure and verify `mail.conceptkid.in` in Resend. For testing before domain verification, use `EMAIL_FROM=ConceptKid <onboarding@resend.dev>`. If email is not configured or sending fails, approval still works and the admin panel will show **Copy Login Instructions** as the fallback.

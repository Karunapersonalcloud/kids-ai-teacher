...
export default function StudentChangePasswordPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok || !data.ok) {
      setMessage(data.error || "Could not update student password.");
      return;
    }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#f7f5ff] p-5">
      <form onSubmit={submit} className="w-full max-w-md rounded-3xl bg-white p-6 shadow-sm">
        <div className="rounded-full bg-amber-100 px-3 py-1 text-center text-xs font-black text-amber-800">Student first login security step</div>
        <h1 className="mt-4 text-3xl font-black text-purple-800">Change Student Password</h1>
        <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">Please choose a new password/PIN before accessing learning content.</p>
